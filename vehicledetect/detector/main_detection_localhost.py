...
# --- Import Elasticsearch client SINKRON ---
from elasticsearch import Elasticsearch

app = FastAPI()

# --- KONFIGURASI UMUM (Duplikasi atau bisa diletakkan di config file terpisah) ---
...
cctv_streams_info = {} # Menyimpan informasi dan status stream CCTV
yolo_models = {} # Menyimpan instance model YOLO per CCTV
...
def post_detection_to_backend(cctv_name_from_detection: str, vehicle_type: str, direction: str):
    def send():
        ...
        try:
            ...
            elastic_index_name = f"{ELASTICSEARCH_INDEX_PREFIX}" 
...
# Masukkan semua fungsi `ffmpeg_reader_generator`, `opencv_reader_generator`, `imageio_reader_generator`
# tapi pastikan semua kode fungsi tersebut ada di sini.
...
def run_detection_and_update_frame_buffer(cctv_name: str):
    ...
    try:
        if cctv_name not in yolo_models:
            ...
            yolo_models[cctv_name] = YOLO('best.pt').to('cuda') # Pastikan CUDA terinstal dan berfungsi jika menggunakan .to('cuda')
    ...
    while cctv_info["is_running"]:
        try:
            ...
            for frame in reader:
                ...
                if results and results[0].boxes.id is not None:
                    for box, obj_id_tensor, cls_id_tensor in zip(results[0].boxes.xyxy, results[0].boxes.id, results[0].boxes.cls):
                        ...
                        if prev_point and is_crossing_line(prev_point, curr_point, p1, p2):
                            line_color = (0, 255, 0)
                            # PENTING: Untuk deteksi IN/OUT yang akurat, Anda perlu LOGIKA LEBIH LANJUT
                            # untuk menentukan apakah objek bergerak dari satu sisi garis ke sisi lain,
                            # bukan hanya melintasi. Ini adalah contoh sederhana.
                            # Anda mungkin ingin menggunakan `cv2.pointPolygonTest` atau perhitungan vektor.
                            
                            # Contoh sederhana: Jika melintasi, kirim dua event (IN dan OUT)
                            # Ini bisa jadi tidak akurat untuk kebutuhan sebenarnya
                            # Anda harus memodifikasi ini sesuai kebutuhan arah Anda.
                        ...
                        color_map = {
                            "Mobil": (0, 204, 255), "Motor": (255, 153, 0),
                            "Bus": (255, 51, 51), "Truk": (102, 255, 102)
                        }
                ...
                cv2.line(frame_copy, p1, p2, line_color, 1) # Gambar garis batas

                with cctv_info["lock"]:
                    cctv_info["latest_frame"] = frame_copy

                time.sleep(0.01) # Sesuaikan kecepatan frame
            
            if cctv_info["is_running"]: # Jika loop reader selesai tapi is_running masih True, coba reconnect
                print(f"[{cctv_name}] Stream ended unexpectedly. Attempting to reconnect...")
                time.sleep(2)
            else:
                break # Jika is_running False, berarti memang diminta berhenti
            ...
            if cctv_info["is_running"]:
                ...
                break # Jika is_running False, keluar dari loop
...
async def video_feed(cctv_name: str):
    ...
    async def generate_frames_for_http():
        while cctv_info["is_running"]:
            ...
            if frame is None:
                await asyncio.sleep(0.1) # Tunggu sebentar jika frame belum ada
                continue
            ...
            await asyncio.sleep(0.03) # Kontrol kecepatan stream ke klien

    try:
        # Jika thread deteksi belum berjalan, mulai sekarang
        if not cctv_info["is_running"]:
            ...
            await asyncio.sleep(1) # Beri waktu thread untuk mulai dan mendapatkan frame pertama
...
async def startup_event():
    print("Detection Backend starting up...")
    try:
        ...
        for cam in cameras_config:
            ...
            cctv_streams_info[cctv_name] = {
                ...
                "is_running": False, # Akan diset True saat video_feed diakses atau di startup
                "latest_frame": None,
                "lock": threading.Lock(),
            ...
            # Opsional: Mulai thread deteksi untuk semua CCTV saat startup
            # Jika Anda ingin semua CCTV langsung aktif begitu backend nyala
...
if __name__ == "__main__":
    import uvicorn
    # Jalankan Detection Backend di port 8001
